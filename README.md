...
When `spawn` omits both `harness` and `profile`, the orchestrator uses a pure, explainable routing policy over configured roles, the selected profile's executable/runtime availability, profile mode, requested effort, and nested-agent capability. Advisory, research, review, and challenge work prefers Pi. Builder work and `requiresSubagents: true` prefer direct Codex, then direct Claude. A caller-supplied harness or profile always wins; an explicit Codex/OpenAI or Claude/Anthropic model identifier selects the corresponding direct harness unless a harness/profile was supplied. OpenCode is explicit-only and is never selected automatically. `agent_fleet({ action: "route", ... })` previews the exact selection, profile/mode, permission profile, ranking, warnings, and exclusions without launching a worker. The policy is configurable under `routing.preference`, `routing.explicitOnly`, `routing.roles`, and `routing.capabilities.requiresSubagents`. Role preset harnesses remain leading preferences. For old configuration files without a `routing` block, an explicitly configured `defaultHarness` is synthesized at the front of the migrated base preference; once `routing.preference` is present, it is authoritative and `defaultHarness` is only a final compatibility fallback. Automatic routing evaluates the explicit, role-preset, or `defaultProfiles` entry for each harness rather than silently choosing another profile; pass `profile` or change `defaultProfiles` to use an alternative. Portable role instructions follow an availability fallback, but harness-specific profile/model/effort values do not; callers should select an explicit harness/profile or supply explicit instructions when wording depends on one harness.
...
Use Ralph loops for substantial iterative delegation that benefits from periodic context resets, not for quick one-shot work. After spawning coworkers, arrange a bounded `return_on` watcher or timed check-in with a completion condition and timeout; Intercom delivery alone must not be treated as a manager wake-up mechanism.
