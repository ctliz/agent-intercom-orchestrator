...
- Restore strict-schema Boss compatibility by accepting `requirements: null` as explicit absence for non-create actions, report the fixed independent-Pi-peer role topology and configured role model/effort in `doctor`, clarify that native harness subagents/per-run overrides are unavailable, and expose the additive `pendingDecision` through tool details.
