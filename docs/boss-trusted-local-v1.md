...
Boss state consumes complete WorkerStore lifecycle snapshots at command handling, session startup, and ordinary lifecycle heartbeats. It records only state transitions, changed failure details, or new activity evidence, caps retained observations, treats a missing exact assigned incarnation as advisory `lost`, and marks an unexpectedly failed, lost, or stopped active-run participant as a failed run. Status labels WorkerStore `ready` and related lifecycle values as process/transport state, never as proof of productive work. For each exact owned assignment it exposes only the dedicated authenticated inbound Intercom timestamp recorded by WorkerStore after sender/owner verification. General lease activity remains separate: manual `renew`, adoption, and lifecycle initialization cannot satisfy first action. Any authenticated timestamp already present when staffing binds is retained only as a baseline; a later timestamp from that same incarnation satisfies the ten-minute first-action deadline. An active assignment with no later timestamp becomes `first_action_stale`; legacy history without a retained exact launch observation reports the deadline unavailable instead of inventing one. Intercom traffic does not prove source edits, tool calls, or substantive output, all of which remain unavailable telemetry.
