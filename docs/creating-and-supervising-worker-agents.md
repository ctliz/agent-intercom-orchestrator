...
Explicit models matching ordered `routing.modelRouting.rules` choose a direct harness; patterns are exact or use one safe trailing `*`. An optional `unmatchedHarness` handles every unmatched explicit model, while `null` preserves normal role routing. `stripPrefixes` controls direct-CLI normalization. `routing.explicitOnly` is authoritative: OpenCode is included by default, but an intentional change is honored. The preview explains every rank, warning, exclusion, selected profile/mode, and permission profile. Harness-specific role profile/model/effort settings do not cross a harness fallback. Role instructions cross only when `routing.fallback.preserveRoleInstructions` is enabled; explicit caller instructions always survive.

The default guidance recommends a bounded Ralph loop for substantial iterative assignments and a bounded `return_on` watcher or timed completion/timeout check-in after spawn. These recommendations are independently configurable under `supervision`; changing them does not alter leases, checkpointing, cleanup, permissions, or ownership.
